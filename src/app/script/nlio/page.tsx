...
export default function NlioStubCorrectionPage() {
  ...
  async function handleVerify() {
    ...
    try {
      const res = await apiRequest<VerifyResponse>('/script/nlio/verify', {
  ...
  async function handleCorrect() {
    ...
    try {
      const res = await apiRequest<CorrectResponse>('/script/nlio/correct', {
        method: 'POST',
        body: JSON.stringify({
          expected_currentno: stub.currentno,
          target_currentno: target,
        }),
        headers: { 'X-Script-Token': token },
      });
                ...
                <table className="table">
                  <tbody>
                    ...
                    <tr>
                      <th>Current No</th>
                      <td>
                        <strong>{stub.currentno}</strong>
                      </td>
                    </tr>
                    <tr>
                      <th>Next Docno</th>
                      <td>
                        <span className="pill">{stub.next_docno}</span>
                      </td>
                    </tr>
                    <tr>
                      <th>Stub Range</th>
                      <td>{stub.currentno} – {stub.stubto}</td>
                    </tr>
                    <tr>
                      <th>Last Updated</th>
                      <td className="muted">{stub.date_updated}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          >
            ...
            <div className="stack" style={{ gap: '10px' }}>
              ...
              {correctResult && (
                <div className="status-card success" style={{ padding: '12px' }}>
                  ...
                  <p className="helper" style={{ margin: '4px 0 0' }}>
                    Next docno: <strong>{correctResult.stub_after.prefix}{String(correctResult.new_currentno).padStart(7, '0')}</strong>
                  </p>
                </div>
              )}
              ...
              <button
                ...
                disabled={correcting || !stub || !targetNo}
              >
